...
struct semaphore sleeping_threads;
...
/* Sleeps for approximately TICKS timer ticks.  Interrupts must
   be turned on. */
void timer_sleep (int64_t ticks1)
{
  int64_t start = timer_ticks ();
  int64_t total_time = start + ticks1;

  ASSERT (intr_get_level () == INTR_ON);


  /*Puts thread onto semaphore waiting list and yields to a new thread*/
  struct thread *cur = thread_current();
  cur->sleep_time = total_time;
  list_push_back(&sleep_list, &cur->sleep_elem);
  cur->ifSleep = true;
  cur = list_entry(&cur->sleep_elem, struct thread, sleep_elem);
  thread_yield();
  sema_down(&cur->sleeping_semaphore);
  cur->ifSleep = false;
...
// Decrements all of threads' sleep_remaining
